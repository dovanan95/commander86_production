...
app.post('/chat_peer', authenticateAccessToken, async function (req, res) {
    try {
        var dbo = mongoUtil.getDb();
        var chatBlocks = await dbo.collection('privateMessage').find({
            $or: [{ 'sender': req.body.my_ID, 'receiver': req.body.partner_ID },
            { 'sender': req.body.partner_ID, 'receiver': req.body.my_ID }]
        }).sort({ 'timestamp': -1 }).limit(req.body.limit).toArray();
...
app.post('/verifyMessBlockchain', authenticateAccessToken, async function (req, res) {
    try {
        ...
        if (update_Mess_json.docType == 'private_message') {
            await dbo.collection('privateMessage').updateOne({ 'messID': req.body.messID }, {
                $set: {
            ...
            await dbo.collection('groupMessage').updateOne({ 'messID': req.body.messID }, {
                $set: {
                    'message': update_Mess_json.content,
                    'sender_name': update_Mess_json.sender_name,
                    'sender': parseInt(update_Mess_json.sender),
